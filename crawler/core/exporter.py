    ...
    for source in sources_catalog["sources"]:
        if source["name"] in updated_sources:
            ...
            image_template_filename = (
                template_path + "/" + distribution.lower().replace(" ", "_") + ".yml.j2"
            )
            ...
            for release in source["releases"]:
                ...
                catalog_export = (
                    catalog_export
                    + image_template.render(catalog=release_catalog, metadata=release)
                    + "\n"
                )

            image_catalog_export_filename = (
                local_repository + "/" + distribution.lower().replace(" ", "_") + ".yml"
            )
            # TODO error handling
